...
export function PopupAlert({ message, onClose }: PopupAlertProps) {
  ...
  // Determine if this is a moderation error
  const isModerationError = typeof message === 'string' && (
    message.toLowerCase().includes('bad words') ||
    message.toLowerCase().includes('inappropriate') ||
    message.toLowerCase().includes('toxic') ||
